...
from wca_solvation import setup_table_writer, setup_nonequilibrium_simulation

PROJECT_DIR = Path().resolve()

parser = ArgumentParser(
    prog=Path(__file__).name, usage='%(prog)s [options]',
    description='''Generate noising trajectories to estimate the
                   work done to solvate a monomer in WCA solvent''')
...
parser.add_argument('-input_dir_suffix', type=str, default='noising', metavar=' ',
                    help='basename of input directory from where data is loaded')
parser.add_argument('-output_dir_suffix', type=str, default='denoising', metavar=' ',
...
if __name__ == '__main__':
    ...
    for index in range(args.first_traj_index,
        ...
        simulation, logger = setup_nonequilibrium_simulation(
            parameters, device, random, str(input_trajectory))
