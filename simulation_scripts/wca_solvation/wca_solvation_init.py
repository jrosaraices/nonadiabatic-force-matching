...
from wca_solvation import setup_table_writer, setup_equilibrium_simulation
...
parser.add_argument('-N_timesteps', type=int, default=2_000_000, metavar=' ',
                    help='number of simulation steps (first half spent in burn-in)')
parser.add_argument('-N_dumpevery', type=int, default=20_000, metavar=' ',
...
if __name__ == '__main__':
    ...
    for index in range(args.first_traj_index,
        ...
        output_trajectory = output_path.joinpath(
            f'init{args.output_name_suffix}_{index:04d}.gsd')
        if args.no_overwrites and output_trajectory.exists():
            print(f'File "{str(output_trajectory)}" already exists')
            continue

        # Instantiate simulation
        simulation, logger = setup_equilibrium_simulation(
            parameters, device, random)
        ...
        simulation.run(int(parameters["N_burnin"]) + int(parameters["N_timesteps"]), write_at_start=True)
